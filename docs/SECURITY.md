# tracelab — 安全设计 / Security design

> 面向用户与贡献者的安全说明。tracelab 会经手**真实的模型 API 凭证**（订阅登录态或
> API key），所以"凭证去了哪、什么落了盘、谁能读到"必须写清楚。本文档是这些不变量的
> 单一事实来源；改动凭证/持久化相关代码时请同步更新。最后更新：2026-06-24。

This document describes how tracelab handles credentials. It is written for users
...
tracelab 是一个**纯本地**工具：它在 `127.0.0.1` 上起一个反向代理，把你本机的 coding
...
落盘的主要风险面——所以 tracelab 的设计原则是**凭证永不落盘**（§2）。
...
1. **凭证永不写盘。** 真实 auth（订阅 token、API key、捕获到的请求头）只存在于
   **进程内存**，随进程退出而消失。SQLite 库、`raw/` 原始字节、任何导出文件里都不含
   可用凭证。
2. **API-key 模式下，key 不离开 tracelab 进程。** 用户把 key 交给 tracelab 后，agent
   被启动时只拿到一个**占位符**（`tracelab-proxy-placeholder`）。代理在转发时把占位符
   换成内存里的真 key。key 不进 agent 进程、不进终端、不进 shell history、不落盘。
3. **持久化的 session 路由是非密的。** 为了让 agent 扛过 tracelab 重启，`live_sessions`
...
tracelab 启动 agent 走代理时，凭证有两种来源：
...
- 真凭证**始终在 agent 手里**，tracelab 从不持有。
- 代理会把每次请求的头（含 auth）记进**内存** `headers`，仅供重放复用——内存、不落盘。

### 3.2 API-key 模式（key）

用户在 tracelab UI/CLI 里提供一个真实 API key。

- 真 key 进 tracelab **内存** `inject`；启动 agent 时给它的环境变量是占位符。
- 代理转发时：删掉 agent 带来的占位符头，换成内存里的真 key（`proxy.go`）。
- **代价**：真 key 只在内存。tracelab 重启后它消失（见 §4）。这是不变量 §2.2 的必然
  结果——要让它扛过重启，只能落盘（破坏 §2.1）或把真 key 直接交给 agent（破坏 §2.2），
  两者都不做。

> 这个**故意的非对称**很重要：订阅模式重启后能自动恢复（真凭证在 agent 手里），
> API-key 模式重启后必须**重新输入一次 key**（真凭证只在 tracelab 内存里，已随重启消失）。

---

## 4. 跨重启的 session 重连（re-attach）

抓包/重放要看到改动就得重启 tracelab，但重启会清空内存里的 session 注册表——已经起好的
agent 会拿到 `502 unknown session token`。为减少这个摩擦，tracelab 持久化**非密**的
...
仍需注意（本地工具的固有面，非 tracelab 特有）：数据目录是普通文件，**同用户的任何进程
...
> 为什么单列：tracelab 的数据文件**默认会被分享/开源**（你正在做的就是这件事）。在一个
