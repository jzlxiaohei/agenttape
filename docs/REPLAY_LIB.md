...
> 扛过 agenttape 重启（订阅模式自动恢复，API-key 模式需在 UI 重输一次 key——真 key 从不
> 落盘）。完整的凭证/持久化安全模型见 [`SECURITY.md`](SECURITY.md)。provider
...
但会话内容已替换为虚构的 Tideline CLI 设计讨论，不再包含 AgentTape 自身的分析记录。
...
- **curl 导出**：`handleCaseCurl`（`internal/server/case_curl.go`）两种模式：
  `proxy`（打 agenttape 代理，不含密钥）/ `direct`（直连含 auth，默认打码）。
...
> `seed:cc-compaction` 曾保留一整段分析 AgentTape 自身的真实对话。现已改为**合成会话**：
...
`example@example.com`。逐 token 复查无 `/Users/`、真实邮箱、姓名、`agenttape` 残留。
