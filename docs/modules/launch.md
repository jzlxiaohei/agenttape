...
把一个 coding agent（Claude Code / Codex）**通过 tracelab 代理**拉起来，这样它和模型之间
...
2. **构造启动方式**，把 agent 指向 `…/s/<token>/`：
   ...
   - 完整捕获（含 hook）走 `tracelab launch` CLI（`internal/launcher`）；轻量只抓 HTTP 的
     走"自己运行"的复制粘贴命令。
...
- `launcher/launcher.go`：构造 `tracelab launch` 实际跑的命令（非侵入）。
...
- **API-key 模式：真 key 只进内存**。启动给 agent 的是占位符
  `tracelab-proxy-placeholder`，代理转发时才换成真 key——key 不进 agent 进程、不进终端
